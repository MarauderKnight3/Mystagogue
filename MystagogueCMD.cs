...
namespace Terraria
{
	public class MystagogueCMD
	{
		...
		static MystagogueCMD()
		{
			...
			new MystagogueCMD("researchall", "(No arguments) Researches all items for the creative menu.", delegate()
			{
				for (int i = 0; i < 5088; i++)
				{
					int num = 0;
					...
					int num2 = 0;
					Main.LocalPlayerCreativeTracker.ItemSacrifices.SacrificesCountByItemIdCache.TryGetValue(i, out num2);
					int num3 = Utils.Clamp<int>(num - num2, 0, num);
					if (num3 == 0)
					{
						return;
					}
					int amount = Math.Min(num3, 4000);
					if (!Main.ServerSideCharacter)
					{
						Main.LocalPlayerCreativeTracker.ItemSacrifices.RegisterItemSacrifice(i, amount);
					}
					else
					{
						NetPacket packet = NetCreativeUnlocksPlayerReportModule.SerializeSacrificeRequest(i, amount);
						NetManager.Instance.SendToServerOrLoopback(packet);
					}
					Main.CreativeMenu.MystagogueRefreshAvailableInfiniteItemsList();
				}
			});
